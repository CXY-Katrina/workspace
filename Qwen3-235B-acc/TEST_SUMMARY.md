...
**当前状态：第四轮 v0.23.0 + QuaRot 精度复测已结束。客户端提交 1,319 条，实际落盘 1,314 条：1,271 正确、43 错误（局部精度 96.7275%）。最后一条长请求无进展而停止客户端；v0.23.0 三机服务保留。**
...
第三轮已停止客户端；完整路径、配置、审计和原始样本见 [第三轮报告](local-quarot-20260920/README.md)。

## 10. 第四轮：v0.23.0 镜像 + QuaRot 权重（2026-09-20）

使用 `quay.io/ascend/vllm-ascend:v0.23.0-a3`，模型路径为 `/mnt/weight/Qwen3-235B-A22B-w8a8-QuaRot`，评测参数和三机拓扑不变。全部 1,319 条请求已提交且无请求失败；JSONL 实际保存 1,314 条，按原后处理得到 **1,271/1,314 = 96.727549%**。这不是完整 1,319 条最终分数；另有 5 条未写入结果文件，其中进度器显示 1,318 条完成，最后一条长请求无进展。

1,314 条中未发现 `�`、连续重复字符、未闭合思考或完整回答重复。10 条重复短语启发式命中经人工检查属于推理中的题干/“Wait”重复，最终答案格式正常。相较上一轮源码环境的 60 条连续字符和 56 条未闭合思考，本轮未复现该退化。详见 [v0.23.0 复测报告](local-v023-quarot-20260920/README.md)。

## 11. EPLB 因果对照（2026-09-20，进行中）

当前使用三机 `qwen235-acc-20260919` 源码容器和 QuaRot 权重进行诊断；基线容器已停止。前述服务器/容器状态属于各轮历史快照，最新状态以本节为准。

已复现新版第 100 条持续感叹号，以及故障服务中第 12 条的快速退化。关闭动态 EPLB 后 8/8 正常；保持 EPLB 开启但将重排触发延后后，24/24 正常。临时打点组也通过了 8 条，但打点可能改变时序，不能视为修复。

**尚未确认 EPLB 因果关系。** 无打点恢复原 4000 步周期后，24/24 条正常，期间发生多次专家更新。随后 60 条历史 bad case 的 64 并发重复实验按用户新指令停止，保存 162 条正常结束记录，未完成计划的 180 条。正常 Wait 和题干重复忽略。

临时源码打点已清除，没有永久修复或修复后的全量分数。具体版本、对照限制及原始证据见 [EPLB 调查](eplb-investigation-20260920/README.md)。

## 12. 关闭 EPLB 的原方法全量测试（2026-09-20，已完成）

按用户最新指令停止探索性重放，三台机器继续使用源码容器、QuaRot 权重和原 AISBench 测试方式，仅将两个 Decode 的 `dynamic_eplb` 改为 false。GSM8K 1319 条、batch_size=64、max_out_len=32768、temperature=0.6、top_p=0.95、thinking=true；保留 `--dump-eval-details`。

全量 **1273/1319 = 96.512509%**，46 条错误，1319 个唯一样本全部保存，请求全部成功，退出码 0。33 个 rank 启动日志确认动态 EPLB 关闭，未发现本轮专家重排日志。

没有乱码、连续感叹号或完整回答重复。13 条短语重复命中中，11 条是可忽略的题干/Wait 推理重复；另外 **1052、1078 两条仍出现持续循环**，都达到 32768 token 上限且思考未闭合。1052 反复输出 `90 + 100 = 190? No. Wait,`，1078 连续输出 `Hmm.`。两题在 v0.23.0 基线均正常答对。因此，EPLB 开启不是所有重复问题的必要条件，关闭开关也不是完整修复；此前连续感叹号的原因仍待独立定位。

双方共同的 1314 条样本，本轮正确 1269 条（96.575342%），基线正确 1271 条（96.727549%）；净少 2 题。原始 prompt 和 gold 逐条一致，基线缺失 5 条，不能直接当作全量分数比较。

新结果目录为 `/workspace/run05-noeplb-quarot-20260920`。当前三台服务器仍使用 `qwen235-acc-20260919` 源码容器，推理服务保留，AISBench 已退出；v0.23.0 容器停止。无永久代码修改。完整版本、配置、逐条审计、异常原文和证据包见 [本轮全量报告](local-noeplb-quarot-20260920/README.md)。
