...
更新日期：2026-09-19。用例：`tests/e2e/nightly/multi_node/external_dp/config/QWEN3_235B_PD.yaml`。

**当前状态：AISBench 已按用户要求停止；三台 v0.23.0 推理服务保留运行。两轮本地测试均提前结束，没有完整 1,319 条的最终精度结果。**
...
|---|---|---|---|---|
| 197 | qwen235-acc-20260919 | 34b5e77cda81 | 第一轮源码安装环境；后作为第二轮 AISBench 独立客户端 | 已停止 |
| 198 | qwen235-acc-20260919 | cc188442d6f6 | 第一轮 Decode | 已停止 |
| 159 | qwen235-acc-20260919 | ffc36594c961 | 第一轮 Decode | 已停止 |
| 197 | qwen235-acc-v023-20260919 | 543625899cde | 第二轮 Prefill + 代理 | 运行中 |
| 198 | qwen235-acc-v023-20260919 | b347439d7874 | 第二轮 Decode rank 0–15 | 运行中 |
| 159 | qwen235-acc-v023-20260919 | 9b45424ead67 | 第二轮 Decode rank 16–31 | 运行中 |

旧容器的 Exited (137) 来自本次主动停止容器，不应作为模型崩溃证据。当前容器仍运行不代表测试还在继续；AISBench 客户端所在旧容器已停止。
...
- 服务模型名：`/workspace/model-official`。

汇总时已核查 33 个服务 `/health` 和代理 `/healthcheck`，**34 / 34 返回 HTTP 200**。这只是可用性检查，不代表精度或文本质量通过。
