set -euo pipefail
while [ ! -f /workspace/run07-mrv2-noeplb-latest-20260920/services.ready ]; do sleep 10; done
export BENCHMARK_HOME=/workspace/benchmark
export LOG_PREFIX=/workspace/run07-mrv2-noeplb-latest-20260920
export no_proxy=localhost,127.0.0.1,178.27.4.197,178.27.4.198,178.27.4.159
unset http_proxy https_proxy HTTP_PROXY HTTPS_PROXY ALL_PROXY all_proxy
mkdir -p /workspace/run07-mrv2-noeplb-latest-20260920/client
cd /workspace/run07-mrv2-noeplb-latest-20260920/client
python -u - <<'PY'
import json
...
Path('/workspace/run07-mrv2-noeplb-latest-20260920/benchmark-results.json').write_text(json.dumps(results, default=str, indent=2))
print('AISBENCH_MRV2_NOEPLB_FULL_COMPLETED', flush=True)
PY
